...
**This is a template. Copy it into your own project's `CONTRIBUTING.md` and edit it.**

Replace anything in `[square brackets]` with your project's specifics, and delete the sections that do not apply to you. Nothing here is required verbatim — a shorter version you actually maintain beats a complete one you do not.

Pair it with [`ACCESSIBILITY.md`](../ACCESSIBILITY.md), which states your conformance target and known limitations. This file tells contributors *how to contribute* accessibility work; that one tells them *what the project is aiming at*.
...
This project aims to be usable by people with disabilities and by people using assistive technologies, keyboard-only navigation, zoom, reflow, captions, reduced motion, or other alternate interaction modes. Accessibility work is treated as a core quality requirement for `[your user-facing code]`, `[any backend behavior that affects user-facing flows]`, and project documentation.
...
- Fixing semantic HTML, labels, headings, landmarks, alt text, and link purpose.
...
- Updating generated content, templates, form responses, API-backed UI states, or rendered output that affects accessibility.
...
4. Map the issue to WCAG success criteria if you can. If you cannot, contribute anyway and say so; a clear description of the barrier is worth more than a wrong citation.
...
Use the best setup available to you and document it in the PR. Suggested combinations:
...
Automated checks are useful, but they do not replace manual testing. A passing axe run is not evidence that a screen reader announces the right thing.

## Opening the PR

Use the accessibility PR template for accessibility-related changes.

A strong accessibility PR includes:
...
## Coding expectations for user-facing code

When contributing to `[your frontend]`, prefer:
...
## Coding expectations for backend behavior

For `[your backend]` services, templates, or server-rendered output that affect user experience:
...
Changes to docs should also be accessible. Please use:
